...
public class CellVisualizer : MonoBehaviour
{
    ...
    public void SetEntropyDebugColor(float entropy)
    {
        if (_renderer == null)
            _renderer = GetComponent<SpriteRenderer>();

        float t = Mathf.Clamp01(entropy);
        Color color = Color.Lerp(Color.gray, Color.blue, t);
        _renderer.color = color;
    }
    public void SetEntropyOverlay(float entropy)
    {
        float e = Mathf.Clamp01(entropy);
        Color tint = new Color(0f, 0f, 1f, e * 0.4f); // blue tint, semi-transparent
        _renderer.color = Color.gray + tint;
    }
