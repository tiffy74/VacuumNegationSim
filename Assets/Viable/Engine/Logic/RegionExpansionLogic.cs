...
{
    /// <summary>
    /// Logic for active region expansion (substrate where state transitions are defined).
    /// Extracted from Assets/Scripts/Events/RegionExpansion.cs (Phase 4).
    /// </summary>
    public static class RegionExpansionLogic
    {
        /// <summary>
        ...
        /// Uses deterministic RNG from StepContext.
        /// </summary>
            ...
            for (int y = 0; y < s.H; y++)
            {
                for (int x = 0; x < s.W; x++)
                {
                    ...
                    for (int d = 0; d < 4; d++)
                    {
                        ...
                        // Probabilistic expansion (deterministic)
                        if (rng.NextDouble() > regionExpansionChance)
                            continue;
                        ...
                        if (seedResourceOnExpansion)
                            s.ResourceLocal[i] = Math.Max(s.ResourceLocal[i], seedResource);
